...
- [ ] Layout + дизайн-тема
- [ ] Список сотрудников — поиск, роль/грейд
- [ ] Профиль: карьерная траектория (grade ladder Junior→Lead, текущая позиция)
- [ ] Профиль: навыки vs требования следующего грейда (разрывы подсвечены)
- [ ] Профиль: таймлайн истории активностей (статусы, оценки)
- [ ] Рекомендации: кнопка «Получить рекомендацию» со skeleton-загрузкой
- [ ] Рекомендации: карточки событий (название, тип, factor-бейджи, обоснование, пометка source llm/fallback)
- [ ] Complete-flow: кнопка «Отметить выполненной» → дельта прогресса по навыкам и готовности к грейду
- [ ] HR-дашборд (`/hr`): проседающие навыки, сотрудники без рекомендованного шага, участие по активностям
- [ ] Admin: форма `/admin/upload` (employees.json + activity_history.csv)
- [ ] Переключатель роли сотрудник/HR в шапке
