...
1. В режиме «Сотрудник» откройте `/employees/E0001`: карьерная лестница Junior → Lead, текущие навыки, разрывы до Middle и история с оценками. Сотрудник по умолчанию — E0001; другой профиль недоступен. Идентификатор хранится в сессии как `employee_id`, роль — `role` (`employee` / `hr`). Это демонстрационная модель без паролей.
...
4. Переключитесь в HR через форму в шапке. Теперь доступны все сотрудники, поиск и фильтры по роли/грейду, `/hr` и `/admin/upload`. На HR-экране проверьте разрывы, сотрудников без доступного полезного шага и доли статусов участия.
