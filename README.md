...
Откройте `/employees` (корень приложения также ведёт на список). Для ручной установки:
...
Blade-страницы используют собранные Vite-ресурсы. После изменения интерфейса
запустите `npm run build` либо оставьте `npm run dev` работающим во время разработки.

### Учётные записи и доступ

После миграций и импорта администратор создаёт аккаунты в доверенной серверной консоли:
...
В Docker используйте префикс `docker compose exec app`. Команда запрашивает пароль
скрытым вводом (не менее 12 символов и не более 72 байт); готовых аккаунтов и паролей по умолчанию нет.
Email должен быть уникальным, профиль сотрудника можно привязать только к одному
аккаунту. HR-аккаунту также можно назначить свой профиль через `--employee`.

Вход — `/login`. Без входа данные недоступны. Роль и идентификатор сотрудника
читаются из аккаунта в БД, а не из полей запроса или старой сессии. Сотрудник видит
только свой профиль; аккаунт без привязки не получает профиль по умолчанию.
Переключатель в шапке меняет режим интерфейса только для назначенных HR-пользователей.
Изменение серверной роли применяется при следующем запросе, включая уже открытые сессии.
При обновлении существующей установки выполните `php artisan migrate --force`;
старый демонстрационный переключатель больше не предоставляет доступ.

## Сценарий для жюри

1. Войдите под созданным аккаунтом сотрудника, привязанным к E0001, и откройте `/employees/E0001`: карьерная лестница Junior → Lead, текущие навыки, разрывы до Middle и история с оценками. Другие профили недоступны.
2. Нажмите «Получить рекомендацию». Во время запроса показывается загрузка; затем появляются до трёх карточек с факторами, объяснением и источником `llm` / `fallback`. Без API-ключа сценарий работает через fallback. Карточки сохраняются при обновлении страницы.
3. На карточке нажмите «Отметить выполненной». Интерфейс показывает изменение уровней навыков и готовности к следующему грейду, обновляет профиль и историю без перехода на JSON. Устаревшие рекомендации очищаются; запросите новые с учётом прогресса. Для активности вне рекомендаций есть отдельная форма завершения; уже завершённые события исключены из неё, кроме повторяемого EV_036.
4. Выйдите и войдите под созданным HR-аккаунтом. Теперь доступны все сотрудники, поиск и фильтры по роли/грейду, `/hr` и `/admin/upload`. На HR-экране проверьте разрывы, сотрудников без доступного полезного шага и доли статусов участия. Для HR с привязанным профилем переключатель в шапке позволяет также проверить личный режим.
5. Загрузите `employees.json` и/или `activity_history.csv`. Профили имеют оболочку `{"employees": [...]}`; CSV — заголовок как в стартовом ките. Новые записи добавляются, существующие обновляются по ID. Обе загрузки выполняются одной транзакцией; ошибка отменяет весь merge. Форма показывает количества импортированных записей. Сначала импортируйте стартовый каталог, затем проверочные данные.
6. Найдите загруженного сотрудника в HR, откройте профиль и заново получите рекомендацию. Импорт профиля/истории очищает рекомендации затронутых сотрудников, импорт каталога — всех. HR-аналитика пересчитывается при каждом запросе.
