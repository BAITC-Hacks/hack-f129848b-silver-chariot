...
- Вход по email и паролю через штатную сессионную аутентификацию Laravel; все маршруты с данными требуют входа
- Администратор создаёт аккаунты через `php artisan users:create`; роль `users.role` и привязка `users.employee_id` назначаются только на сервере, публичной регистрации нет
- Переключатель «Сотрудник / HR» доступен только аккаунтам с назначенной HR-ролью и меняет режим интерфейса; Gate `access-hr` проверяет и роль аккаунта, и режим сессии
- Middleware `EnsureHrRole` на `/hr/*` и `/admin/upload`; сотрудник видит только профиль, привязанный к его аккаунту, без профиля по умолчанию
